import ScreenTime from "block_timetracker/screentime";
import ajax from "core/ajax";

export const init = (contextid, config) => {
  const $timerDisplay = config.showtimer
    ? document.querySelector(".timer-display")
    : null;
  const $timer = config.showtimer ? document.getElementById("timer") : null;
  const $reportedtime = config.showtimer
    ? document.getElementById("reportedtime")
    : null;
  const $inactivitytime = config.showtimer
  ...
  const screentime = new ScreenTime({
    ...
    onReport: async (log) => {
      ...
      ajax.call([
        {
          methodname: "block_timetracker_update_register",
          args: {
            timespent: log.body,
